...
namespace FileHistory
{
    public partial class MainForm : Form
    {
        ...
        IBackupScheduler _backupScheduler;

        /// <summary>
        /// コンストラクタ
        /// </summary>
        public MainForm(Settings settings, IBackupDb db, ILoggerFactory loggerFactory, Crawler crawler, IBackupScheduler backupScheduler)
        {
            _settings = settings;
            _db = db;
        ...
        void ApplyLocalization()
        {
            ...
            button_suspend.Text = Strings.Get("MainForm_ButtonSuspend");
            button_buckup_schedule.Text = Strings.Get("MainForm_ButtonSchedule");
        }
        ...
        bool _crawlingSuspended { get; set; } = false;
        /// <summary>
        /// クローリング一時停止/再開
        /// </summary>
        /// <param name="sender"></param>
        ...
        {
            if (_crawlingSuspended)
            {
                button_suspend.Text = Strings.Get("MainForm_ButtonSuspend");
                Interlocked.Decrement(ref _crawler.CrawlingSuspended);
                _crawlingSuspended = false;
            }
            else
            {
                button_suspend.Text = Strings.Get("MainForm_ButtonResume");
                Interlocked.Increment(ref _crawler.CrawlingSuspended);
                _crawlingSuspended = true;
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void button_buckup_schedule_Click(object sender, EventArgs e)
        {
            new BackupScheduleForm(_backupScheduler).ShowDialog();
        }
    }
}
