        ...
        static void Main()
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    var menuCrawlNow = new ToolStripMenuItem { Text = Strings.Get("Tray_CrawlNow") };
                    menuCrawlNow.Click += (s, e) =>
                    {
                        logger?.LogInformation($"Menu-CrawlNow by user operation.");
                        _crawler?.TriggerNow();
                    };
