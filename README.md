...
**Option A — download:** grab a zip from [Releases](../../releases) (`-standalone` needs no .NET runtime), unzip, run `FileHistory.exe`.
...
That's it — the app sits in the system tray and starts protecting your `Documents` folder, saving versions to `%USERPROFILE%\FileHistoryCloneBackup`. Edit [appsettings.json](FileHistory/appsettings.json) to choose your own folders (see [appsettings.example.json](FileHistory/appsettings.example.json) for a full annotated example).

## Motivation

Windows 8 introduced [File History](https://support.microsoft.com/en-us/windows/file-history-in-windows-5de0e203-ebae-05ab-db85-d5aa0a199255), a wonderfully simple "set it and forget it" feature: point it at a drive, and every version of every document is kept automatically. Unfortunately, Microsoft has effectively stopped maintaining it — it was removed from Settings in favor of cloud backup, long-standing bugs (silently stopped backups, files skipped without notice) remain unfixed, and its future in Windows is uncertain.
...
| Scan Now | Trigger a full crawl immediately |
