...
(更新は `winget upgrade tomoyukioya.FileHistoryClone`)

**方法 B — ダウンロード:** [Releases](../../releases) からインストーラ(`FileHistoryCloneSetup-*.exe`、ユーザー単位・管理者不要)を実行 — または portable な zip(`-standalone` 版は .NET ランタイム不要)を展開して `FileHistory.exe` を実行。
...
**設定ウィンドウ**(トレイアイコン →**「設定を開く」**、初回起動時にも自動表示)から、バックアップの保存先・保護対象フォルダ・除外パターン・タイミング・保持ポリシーを、ファイルを手編集せずに指定できます。指定内容は `appsettings.json` に書き出されます。

上級者は `appsettings.json` を直接編集することもできます(`//` コメントも使えます)。ファイルを直接編集した場合は、アプリを再起動すると反映されます。
...
| キー | 説明 |
| --- | --- |
| `BackupBaseDir` | バックアップ先のルート。実データは `{BackupBaseDir}\{ユーザー名}\{マシン名}\BackupFiles` 配下に保存 |
...
| `CrawlingInterval` | フルクロールの実行周期(秒)。前回クロール開始から次のクロール開始までの時間(既定 1 日) |
| `CrawlingIdleTimer` | クロールを開始するのに必要なユーザー無操作時間(秒) |
| `Language` | UI 言語(`"ja"`、`"en"`)。空なら OS に追従 |
| `MaxGenerations` | 1ファイルあたりの最大保持世代数(0 = 無制限)。新しいバックアップの保存時に即時適用 |
| `RetentionDays` | この日数より古いバックアップを削除。最新世代は常に保持(0 = 無制限) |
| `RetentionScanInterval` | 保持ポリシー適用スキャンの実行間隔(秒)。`RetentionDays` と遡及適用を担当 |
| `RetentionStartupDelay` | 起動から最初の保持ポリシースキャンまでの待機秒数 |
| `MaxLowPrioritySchedules` / `MaxCopyWorkers` | 上級者向け: クロール由来バックアップの待ち行列上限 / コピーの同時実行数 |
...
- **BackupScheduler** はファイルがバックアップ間隔の間更新されていないことを確認してから、設定可能な並列数でコピーします。失敗したコピーはロールバックされます。コピー成功のたびに、そのファイルへ保持ポリシーを即時適用します。
- **RetentionWorker** が保持ポリシーを定期適用します(日数制限と、更新されなくなったファイルの整理)。
